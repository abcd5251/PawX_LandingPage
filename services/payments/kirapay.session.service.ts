...
export async function findInvoiceById(executor: DbExecutor, invoiceId: string) {
...
export async function findInvoiceByIdentifier(executor: DbExecutor, identifierInUsd: string) {
  const result = await executor.execute(sql`
    SELECT
      id,
      user_id,
...
export async function findKiraPayTransactionByReferences(references: Set<string>) {
...
function transactionMatchesInvoice(
  ...
  const stringValues = [
    ...
    firstStringValue(transaction, ['paymentLink.url', 'url', 'data.url'])
  ]
...
async function findKiraPayPaidTransactionForInvoice(invoice: InvoiceRow, link: KiraPayLinkItem | null) {
...
export async function persistSuccessfulPayment(
  ...
  const paymentDetails = getWebhookPaymentDetails(payload, invoice)
  const paidAt = getWebhookPaidAt(payload)
  ...
  `)
  let payment = insertedRows[0] as PaymentUpsertRow | undefined

  if (!payment) {
    const existingRows = await executor.execute(sql`
      SELECT id, matched_invoice_id
      FROM payments
      WHERE tx_hash = ${paymentDetails.txHash}
      LIMIT 1
    `)
    const existing = existingRows[0] as PaymentUpsertRow | undefined

    if (!existing) {
      throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Failed to persist payment record')
    }

    if (existing.matched_invoice_id && existing.matched_invoice_id !== invoice.id) {
      log.warn({
        attemptedInvoiceId: invoice.id,
        existingInvoiceId: existing.matched_invoice_id,
        matchedBy,
        message:
          'KiraPay transaction already reconciled with a different invoice — refusing to rebind',
...
export async function syncInvoicePaymentWithKiraPay(invoice: InvoiceRow) {
  ...
  return db('primary').transaction(async (tx: DbExecutor) => {
...
export async function createPaymentSession(
  ...
  if (body.tokenOut.chainId.toLowerCase() === 'sol') {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Solana is not supported for KiraPay payments')
  }

  const receiver = config.kirapayReceiverAddress
